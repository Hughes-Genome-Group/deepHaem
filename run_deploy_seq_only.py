...
Give a sequence window in bed-like format and report its score
    ...
    with open(out_file, "w") as fw:
        with open(FLAGS.input, "r") as infile:
            ...
            while True:
                ...
                # process sequences directtly or get sequence form bed/vvcf windows -----
                chunk_seqs = []
                if FLAGS.do == 'seq':
                    for seq in lines:
                    ...
                    with open(chunk_bed.seqfn, "r") as seqin:
                        ...
                        if FLAGS.do == 'damage':
                            ...
                            for t in tag_list:
                                ...
                                # var position is int(bp_context//2)
                                if tmp_seq[replace_base:(replace_base+tmp_ref_length)][0] == tmp_ref:
                                    tmp_seq[replace_base:(replace_base+tmp_ref_length)] = tmp_var
                                    tmp_seq = ''.join(tmp_seq)
                                    chunk_seqs.append(tmp_seq)  # add to chunk seqs
                                    var_dict[t]['varline'] = len(chunk_seqs) - 1 # reference in diictionary

                                # Else replace reference base with base specifed in vcf and flag as warning
                                else:
                                    tmp_ref_seq = list(chunk_seqs[var_dict[t]['refline']])
                                    tmp_ref_seq[replace_base] = tmp_ref
                                    chunk_seqs[var_dict[t]['refline']] = ''.join(tmp_ref_seq)
                                    tmp_seq[replace_base] = tmp_var
                                    tmp_seq = ''.join(tmp_seq)
                                    chunk_seqs.append(tmp_seq)  # add to chunk seqs
                                    var_dict[t]['varline'] = len(chunk_seqs) - 1 # reference in diictionary
                                    print('Warning replacing reference sequence base %s with ref base %s provided in vcf file for %s. Please encode variants from reference sequence perspective!' % (tmp_seq[replace_base], var_dict[t]['ref'], var_dict[t]['tag']))
