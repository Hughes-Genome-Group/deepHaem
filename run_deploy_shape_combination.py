...
# NOTE 0 based indexed bed like format --> eneed to imlmenet switch between 1 and 0 based
...
flags.DEFINE_integer('bin_size', 1, 'Bin size to apply when running over the new sequence.')
...
def predict(sess,
    ...
    for step in range(batches_to_run):
        ...
        # print(seqs[test_batch_range][0].shape)
    ...
    if remaining > 0:
        ...
        tmp_sigmoid = sess.run(get_sigmoid, feed_dict=feed_dict)
        tmp_sigmoid = np.asarray(tmp_sigmoid)
        tmp_sigmoid = np.squeeze(tmp_sigmoid)
...
with tf.Session(config = config) as sess:
    ...
    with open(FLAGS.input, "r") as rdf:
        ...
        for seq in run_seqs:
            ...
            if seq.shape[0] != FLAGS.bp_context:
                print("Length doesn't match ...")
                print(seq.shape)
                sys.exit()
            hotseqs.append(seq)

        hotseqs = np.asarray(hotseqs, dtype=np.float32)
        ...
        if FLAGS.slize != 'all':
            predictions = predictions[:, slize_scheme]
